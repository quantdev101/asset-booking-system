...
from .forms import StudentRegistrationForm, BookingForm, ResourceForm, BookingStatusForm
...
def resources_list(request):
    ...
    return render(request, 'student/resources.html', {'resources': resources, 'selected_type': resource_type})
...
def book_resource(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            print("Form errors:", form.errors)  # shows in terminal for debugging
    else:
        form = BookingForm()
...
def admin_reports(request):
    resource_usage = Resource.objects.annotate(
        ...
        approved_bookings=Count('bookings', filter=__import__('django.db.models', fromlist=['Q']).Q(bookings__status='approved')),
    )
